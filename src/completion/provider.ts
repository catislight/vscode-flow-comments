  ...
  const completionProvider = vscode.languages.registerCompletionItemProvider({ scheme: 'file' }, {
    async provideCompletionItems(document, position) {
      ...
      const items = token.useSegment
        ? buildSegmentItems(features, prefix, token.body, token.typedSep, token.fullTokenRange)
        : buildFlatItems(features, prefix, token.partial, token.range);
...
function buildSegmentItems(features: string[], prefix: string, body: string, typedSep: string, fullTokenRange: vscode.Range): vscode.CompletionItem[] {
  const r = sepRegex();
...
function buildFlatItems(features: string[], prefix: string, partial: string, range: vscode.Range): vscode.CompletionItem[] {
  const mkRaw = partial;
